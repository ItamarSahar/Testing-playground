...
describe('UserService', () => {
	beforeEach(() => {
		//?Reset the Service and dont make the test depend on each other.
	...
	it('should login successfully', async () => {
		...
		httpService.post.mockResolvedValue(httpResp) //? Async mock
	...
	it('should load user from cache', async () => {
		...
		//! if we dont use the mock reset then it should be call the second time !
		expect(httpService.post).toHaveBeenCalledTimes(1)
		expect(res1).toBe(mockUser2)

		const res2 = await userService.login(mockCred2.username, mockCred2.password)
		//! here we check that the user loaded from cache and not from http request
		expect(httpService.post).toHaveBeenCalledTimes(1)
